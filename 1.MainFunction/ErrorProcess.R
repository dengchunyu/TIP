...
DealWithError <- function(filePath, fileName, codePath, format.of.file, type.of.data, saveDir){
  ...
  #error3: geneNameError Gene identifiers don't match -----------------------------------------------------
       ...
       gene.info <- get(load(sprintf("%s/RCodeAndData/signature/human_gene2ensembl2symbol_list.RData", codePath)));
       ...
       load(sprintf("%s/RCodeAndData/signature/annotation.RData",codePath));
