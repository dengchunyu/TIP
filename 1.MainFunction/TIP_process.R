...
TIP_process <- function(
  ...
  print(paste(codePath,"RCodeAndData/cellSignature/CIBERSORT.server.R",sep="/"))
  print(paste(codePath,"RCodeAndData/signature/immunityScore.server.R",sep="/"))
  source(paste(codePath,"RCodeAndData/cellSignature/CIBERSORT.server.R",sep="/"))
  source(paste(codePath,"RCodeAndData/signature/immunityScore.server.R",sep="/"))
  
  print("start to run immunityScore.server!")
  immunityScore.server(codePath =paste(codePath,"RCodeAndData/signature",sep="/"), 
                       filePath = filePath,
                       fileName = fileName,
                       saveDir = filePath,
  ...
  CIBERSORT.server(codePath =paste(codePath,"RCodeAndData/cellSignature",sep="/"),
                   filePath = filePath,
                   fileName = fileName,
                   signaturePath = paste(codePath,"RCodeAndData/cellSignature",sep="/"),
                   saveDir = filePath,
                   perm = permTimes,
