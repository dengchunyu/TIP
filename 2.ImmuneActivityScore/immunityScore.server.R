...
#' @param codePath the storage path of import code files and underlying .RData
#' @param filePath the storage path of the users expression profile
#' @param saveDir the storage path of results
#' @param sampleNumber number of samples in profile
#' @param permTimes times of permutation, default=100
...
immunityScore.server <- function(codePath, filePath, saveDir, sampleNumber, permTimes = 100, type.of.data,
  ...
  load(sprintf("%s/genename_length3.0.Rdata",codePath)); #length of genes for transformation from count to TPM
	source(sprintf("%s/Count2TPM3.0.R",codePath));
  
	source(sprintf("%s/ProcessMultipleSample.R",codePath));#Main function for expression profile with multiple samples 
	source(sprintf("%s/ProcessSingleSample.R",codePath));#Main function for expression profile with only one sample 
